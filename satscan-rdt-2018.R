...
# local clustering of cholera for 2018
#
...
cases <- read.csv(file = "C:/R-projects/Uvira-spatial-epid/uvira.case.rdt.pos-2018.csv",
...
coordinates <- read.csv(file = "C:/R-projects/Uvira-spatial-epid/uvira.coord.rdt.pos-2018.csv",
                        header = TRUE, fileEncoding="UTF-8-BOM")
coordinates <- coordinates %>% dplyr::select(id,x,y)

class(coordinates)
str(coordinates)
#224   matched coordinates total

#reference population data
#in format <Ave name>, <dummy year>, <population>
population <- read.csv(file = "C:/R-projects/Uvira-spatial-epid/uvira.ave.pop.rdt.pos-2018.csv",
                       header = TRUE, fileEncoding="UTF-8-BOM")
class(population)
str(population)
#confirmed 224 pop entries
...
ss.options(c("StartDate=2018/01/01","EndDate=2018/12/31"))
ss.options(list(CaseFile="cholera.cas", 
                ...
                MonteCarloReps=999,
                ProspectiveStartDate="2018/01/01",
...
#Run analysis
result_uvira.rdt.pos.2018 <- satscan(td, "cholera", sslocation="/Program Files/SaTScan")

saveRDS(result_uvira.rdt.pos.2018, file = "C:/R-projects/Uvira-spatial-epid/result_uvira.rdt.pos.2018.rds") 

write.table(result_uvira.rdt.pos.2018$main, file = "C:/R-projects/Uvira-spatial-epid/retroresrdt2018.txt")

#Summarize cluster results succinctly
summary(result_uvira.rdt.pos.2018)
summary.default(result_uvira.rdt.pos.2018)
result_uvira.rdt.pos.2018$main

#Summarize detailed cluster information and sig clusters
result_uvira.rdt.pos.2018$col
result_uvira.rdt.pos.2018$col$OBSERVED[result_uvira.rdt.pos.2018$col$P_VALUE<0.05]
result_uvira.rdt.pos.2018$col$EXPECTED[result_uvira.rdt.pos.2018$col$P_VALUE<0.05]
result_uvira.rdt.pos.2018$col$POPULATION[result_uvira.rdt.pos.2018$col$P_VALUE<0.05]
result_uvira.rdt.pos.2018$col$RADIUS[result_uvira.rdt.pos.2018$col$P_VALUE<0.05]*1000
result_uvira.rdt.pos.2018$col$REL_RISK[result_uvira.rdt.pos.2018$col$P_VALUE<0.05]
result_uvira.rdt.pos.2018$col$P_VALUE[result_uvira.rdt.pos.2018$col$P_VALUE<0.05]

#Create an output of locations belonging to a cluster
cluster.case.loc <- result_uvira.rdt.pos.2018$gis 
...
cluster.cases.rdt.pos.2018 <- cluster.case.loc %>% 
...
saveRDS(cluster.cases.rdt.pos.2018, file = "C:/R-projects/Uvira-spatial-epid/cluster.cases.rdt.pos.2018.rds")         
...
shapefile(result_uvira.rdt.pos.2018$shapeclust[result_uvira.rdt.pos.2018$shapeclust$P_VALUE <=0.049, ], 
          filename='C:/R-projects/Uvira-spatial-epid/clusters.rdt.pos.2018.shp', overwrite=TRUE)

clusters.rdt.pos.2018.sig <- read_sf('C:/R-projects/Uvira-spatial-epid/clusters.rdt.pos.2018.shp')

(clusters.rdt.pos.2018 <- ggplot() + 
    geom_sf(data=uvira.ave.shape) +
    geom_sf(data=clusters.rdt.pos.2018.sig, colour = "darkorange1", 
            size = 0.9, fill = alpha("#2C77BF", .5)) +
    geom_sf_text(data=clusters.rdt.pos.2018.sig, 
    ...
    labs(subtitle = "B: 2018")
  #labs(title="Spatiotemporal clusters of cholera, 
  #Uvira, Sud Kivu, DRC, 201820")
)

ggsave("C:\\R-projects\\Uvira-spatial-epid\\Uvira_graphs\\satscan_clusters.rdt.pos.2018.jpeg", width = 11.69, height = 8.27,
       dpi="print")
dev.off()  

saveRDS(clusters.rdt.pos.2018, file = "C:/R-projects/Uvira-spatial-epid/clusters.rdt.pos.2018.rds") 

#To save file
#sink(file='C:/R-projects/Uvira-spatial-epid/stss_result.rdt.pos_2018.txt')
