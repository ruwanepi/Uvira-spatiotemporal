...
cases <- read.csv(file = "C:/R-projects/Uvira-spatial-epid/uvira.case-2020.csv",
...
coordinates <- read.csv(file = "C:/R-projects/Uvira-spatial-epid/uvira.coord-2020.csv",
                        header = TRUE, fileEncoding="UTF-8-BOM")
coordinates <- coordinates %>% dplyr::select(id,x,y)

class(coordinates)
str(coordinates)
# 952   matched coordinates total

#reference population data
#in format <Ave name>, <dummy year>, <population>
population <- read.csv(file = "C:/R-projects/Uvira-spatial-epid/uvira.ave.pop-2020.csv",
                       header = TRUE, fileEncoding="UTF-8-BOM")
class(population)
str(population)
...
#Run analysis
result_uvira.2020 <- satscan(td, "cholera", sslocation="/Program Files/SaTScan")

saveRDS(result_uvira.2020, file = "C:/R-projects/Uvira-spatial-epid/result_uvira.2020.rds") 


#Summarize cluster results succinctly
summary(result_uvira.2020)
summary.default(result_uvira.2020)
result_uvira.2020$main

write.table(result_uvira.2020$main, file = "C:/R-projects/Uvira-spatial-epid/retrores2020.txt")
write.csv(result_uvira.2020$col, 'C:/R-projects/Uvira-spatial-epid/retro2020.csv', row.names=FALSE)

#Summarize detailed cluster information and sig clusters
result_uvira.2020$col
result_uvira.2020$col$OBSERVED[result_uvira.2020$col$P_VALUE<0.05]
result_uvira.2020$col$EXPECTED[result_uvira.2020$col$P_VALUE<0.05]
result_uvira.2020$col$POPULATION[result_uvira.2020$col$P_VALUE<0.05]
result_uvira.2020$col$RADIUS[result_uvira.2020$col$P_VALUE<0.05]*1000
result_uvira.2020$col$REL_RISK[result_uvira.2020$col$P_VALUE<0.05]
result_uvira.2020$col$P_VALUE[result_uvira.2020$col$P_VALUE<0.05]

#Create an output of locations belonging to a cluster
cluster.case.loc <- result_uvira.2020$gis 
...
cluster.cases.2020 <- cluster.case.loc %>% 
...
saveRDS(cluster.cases.2020, file = "C:/R-projects/Uvira-spatial-epid/cluster.cases.2020.rds")         
...
shapefile(result_uvira.2020$shapeclust[result_uvira.2020$shapeclust$P_VALUE <=0.049, ], 
          filename='C:/R-projects/Uvira-spatial-epid/clusters.2020.shp', overwrite=TRUE)

clusters.2020.sig <- read_sf('C:/R-projects/Uvira-spatial-epid/clusters.2020.shp')

(clusters.2020 <- ggplot() + 
    geom_sf(data=uvira.ave.shape) +
    geom_sf(data=clusters.2020.sig, colour = "darkorange1", 
            size = 0.9, fill = alpha("#2C77BF", .5)) +
    geom_sf_text(data=clusters.2020.sig, 
...
ggsave("C:\\R-projects\\Uvira-spatial-epid\\Uvira_graphs\\satscan_clusters.2020.jpeg", width = 11.69, height = 8.27,
       dpi="print")
dev.off()  

saveRDS(clusters.2020, file = "C:/R-projects/Uvira-spatial-epid/clusters.2020.rds") 

#To save file
sink(file='C:/R-projects/Uvira-spatial-epid/stss_result_2020.txt')
