...
cases <- read.csv(file = "C:/R-projects/Uvira-spatial-epid/uvira.case-2018.csv",
...
coordinates <- read.csv(file = "C:/R-projects/Uvira-spatial-epid/uvira.coord-2018.csv",
                        header = TRUE, fileEncoding="UTF-8-BOM")
coordinates <- coordinates %>% dplyr::select(id,x,y)

class(coordinates)
str(coordinates)
#951 matched coordinates total

#reference population data
#in format <Ave name>, <dummy year>, <population>
population <- read.csv(file = "C:/R-projects/Uvira-spatial-epid/uvira.ave.pop-2018.csv",
                       header = TRUE, fileEncoding="UTF-8-BOM")
class(population)
str(population)
#confirmed 951 pop entries
...
#Run analysis
result_uvira.2018 <- satscan(td, "cholera", sslocation="/Program Files/SaTScan")

write.table(result_uvira.2018$main, file = "C:/R-projects/Uvira-spatial-epid/retrores2018.txt")
write.csv(result_uvira.2018$col, 'C:/R-projects/Uvira-spatial-epid/retro2018.csv', row.names=FALSE)

saveRDS(result_uvira.2018, file = "C:/R-projects/Uvira-spatial-epid/result_uvira.2018.rds") 

#Summarize cluster results succinctly
summary(result_uvira.2018)
summary.default(result_uvira.2018)
result_uvira.2018$main

#Summarize detailed cluster information and sig clusters
result_uvira.2018$col
result_uvira.2018$col$OBSERVED[result_uvira.2018$col$P_VALUE<0.05]
result_uvira.2018$col$EXPECTED[result_uvira.2018$col$P_VALUE<0.05]
result_uvira.2018$col$POPULATION[result_uvira.2018$col$P_VALUE<0.05]
result_uvira.2018$col$RADIUS[result_uvira.2018$col$P_VALUE<0.05]*1000
result_uvira.2018$col$REL_RISK[result_uvira.2018$col$P_VALUE<0.05]
result_uvira.2018$col$P_VALUE[result_uvira.2018$col$P_VALUE<0.05]

#Create an output of locations belonging to a cluster
cluster.case.loc <- result_uvira.2018$gis 
...
cluster.cases.2018 <- cluster.case.loc %>% 
...
saveRDS(cluster.cases.2018, file = "C:/R-projects/Uvira-spatial-epid/cluster.cases.2018.rds")         
...
shapefile(result_uvira.2018$shapeclust[result_uvira.2018$shapeclust$P_VALUE <=0.049, ], 
          filename='C:/R-projects/Uvira-spatial-epid/clusters.2018.shp', overwrite=TRUE)

clusters.2018.sig <- read_sf('C:/R-projects/Uvira-spatial-epid/clusters.2018.shp')

(clusters.2018 <- ggplot() + 
    geom_sf(data=uvira.ave.shape) +
    geom_sf(data=clusters.2018.sig, colour = "darkorange1", 
            size = 0.9, fill = alpha("#2C77BF", .5)) +
    geom_sf_text(data=clusters.2018.sig, 
...
ggsave("C:\\R-projects\\Uvira-spatial-epid\\Uvira_graphs\\satscan_clusters.2018.jpeg", width = 11.69, height = 8.27,
       dpi="print")
dev.off()  

saveRDS(clusters.2018, file = "C:/R-projects/Uvira-spatial-epid/clusters.2018.rds") 

#To save file
sink(file='C:/R-projects/Uvira-spatial-epid/stss_result_2018.txt')
