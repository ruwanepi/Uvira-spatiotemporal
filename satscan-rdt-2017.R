...
# local clustering of cholera for 2017
#
...
cases <- read.csv(file = "C:/R-projects/Uvira-spatial-epid/uvira.case.rdt.pos-2017.csv",
...
coordinates <- read.csv(file = "C:/R-projects/Uvira-spatial-epid/uvira.coord.rdt.pos-2017.csv",
                        header = TRUE, fileEncoding="UTF-8-BOM")
coordinates <- coordinates %>% dplyr::select(id,x,y)

class(coordinates)
str(coordinates)
#354   matched coordinates total

#reference population data
#in format <Ave name>, <dummy year>, <population>
population <- read.csv(file = "C:/R-projects/Uvira-spatial-epid/uvira.ave.pop.rdt.pos-2017.csv",
                       header = TRUE, fileEncoding="UTF-8-BOM")
class(population)
str(population)
#confirmed 354 pop entries
...
ss.options(c("StartDate=2017/01/01","EndDate=2017/12/31"))
ss.options(list(CaseFile="cholera.cas", 
                ...
                MonteCarloReps=999,
                ProspectiveStartDate="2017/01/01",
...
#Run analysis
result_uvira.rdt.pos.2017 <- satscan(td, "cholera", sslocation="/Program Files/SaTScan")

saveRDS(result_uvira.rdt.pos.2017, file = "C:/R-projects/Uvira-spatial-epid/result_uvira.rdt.pos.2017.rds") 

#Summarize cluster results succinctly
summary(result_uvira.rdt.pos.2017)
summary.default(result_uvira.rdt.pos.2017)
result_uvira.rdt.pos.2017$main

write.table(result_uvira.rdt.pos.2017$main, file = "C:/R-projects/Uvira-spatial-epid/retrores2017.txt")

#Summarize detailed cluster information and sig clusters
result_uvira.rdt.pos.2017$col
result_uvira.rdt.pos.2017$col$OBSERVED[result_uvira.rdt.pos.2017$col$P_VALUE<0.05]
result_uvira.rdt.pos.2017$col$EXPECTED[result_uvira.rdt.pos.2017$col$P_VALUE<0.05]
result_uvira.rdt.pos.2017$col$POPULATION[result_uvira.rdt.pos.2017$col$P_VALUE<0.05]
result_uvira.rdt.pos.2017$col$RADIUS[result_uvira.rdt.pos.2017$col$P_VALUE<0.05]*1000
result_uvira.rdt.pos.2017$col$REL_RISK[result_uvira.rdt.pos.2017$col$P_VALUE<0.05]
result_uvira.rdt.pos.2017$col$P_VALUE[result_uvira.rdt.pos.2017$col$P_VALUE<0.05]

#Create an output of locations belonging to a cluster
cluster.case.loc <- result_uvira.rdt.pos.2017$gis 
...
cluster.cases.2017 <- cluster.case.loc %>% 
...
saveRDS(cluster.cases.2017, file = "C:/R-projects/Uvira-spatial-epid/cluster.cases.rdt.pos.2017.rds")         
...
shapefile(result_uvira.rdt.pos.2017$shapeclust[result_uvira.rdt.pos.2017$shapeclust$P_VALUE <=0.049, ], 
          filename='C:/R-projects/Uvira-spatial-epid/clusters.rdt.pos.2017.shp', overwrite=TRUE)

clusters.rdt.pos.2017.sig <- read_sf('C:/R-projects/Uvira-spatial-epid/clusters.rdt.pos.2017.shp')

(clusters.rdt.pos.2017 <- ggplot() + 
    geom_sf(data=uvira.ave.shape) +
    geom_sf(data=clusters.rdt.pos.2017.sig, colour = "darkorange1", 
            size = 0.9, fill = alpha("#2C77BF", .5)) +
    geom_sf_text(data=clusters.rdt.pos.2017.sig, 
    ...
    labs(subtitle = "B: 2017")
  #labs(title="Spatiotemporal clusters of cholera, 
  #Uvira, Sud Kivu, DRC, 201720")
)

ggsave("C:\\R-projects\\Uvira-spatial-epid\\Uvira_graphs\\satscan_clusters.rdt.pos.2017.jpeg", width = 11.69, height = 8.27,
       dpi="print")
dev.off()  

saveRDS(clusters.rdt.pos.2017, file = "C:/R-projects/Uvira-spatial-epid/clusters.rdt.pos.2017.rds") 

#To save file
#sink(file='C:/R-projects/Uvira-spatial-epid/stss_result_rdt.pos_2017.txt')
